...
class AdvancedSensorAnalyzer extends StatefulWidget {
  const AdvancedSensorAnalyzer({super.key});

  @override
  State<AdvancedSensorAnalyzer> createState() => _AdvancedSensorAnalyzerState();
}

class _AdvancedSensorAnalyzerState extends State<AdvancedSensorAnalyzer> {
  List<SensorDataPoint> _accelerometerData = [];
  List<SensorDataPoint> _gyroscopeData = [];
  List<TapEvent> _detectedTaps = [];
  StreamSubscription<UserAccelerometerEvent>? _accelerometerSubscription;
  StreamSubscription<GyroscopeEvent>? _gyroscopeSubscription;
  bool _isCollecting = false;
  double _detectionThreshold = 6.0;
  int _analysisWindowMs = 500;
  ...
  void _startCollection() {
    ...
    _accelerometerSubscription = userAccelerometerEvents.listen((event) {
      ...
      setState(() {
        _accelerometerData.add(SensorDataPoint(
          timestamp: timestamp,
          x: event.x,
          y: event.y,
          z: event.z,
        ...
        // Maintain only recent data (last 10 seconds)
    ...
    _gyroscopeSubscription = gyroscopeEvents.listen((event) {
      ...
      setState(() {
        _gyroscopeData.add(SensorDataPoint(
          timestamp: timestamp,
          x: event.x,
          y: event.y,
          z: event.z,
  ...
  void _maintainDataWindowSize() {
    ...
    _accelerometerData.removeWhere((point) => point.timestamp.isBefore(cutoffTime));
    _gyroscopeData.removeWhere((point) => point.timestamp.isBefore(cutoffTime));
  }
  
  void _detectTaps() {
    if (_accelerometerData.length < 3) return;
    
    // Look for peaks in the recent data
    final recentData = _accelerometerData.length > 10 
        ? _accelerometerData.sublist(_accelerometerData.length - 10)
        : _accelerometerData;
    ...
    for (int i = 1; i < recentData.length - 1; i++) {
      ...
      if (current.magnitude > _detectionThreshold && 
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        const Text('Analysis Window:'),
                        const SizedBox(width: 10),
                        Expanded(
                          child: Slider(
                            value: _analysisWindowMs.toDouble(),
                            min: 100,
                            max: 2000,
                            divisions: 19,
                            label: '${_analysisWindowMs}ms',
                            onChanged: (value) {
                              setState(() {
                                _analysisWindowMs = value.toInt();
                              });
                            },
                          ),
                        ),
                        Text('${_analysisWindowMs}ms'),
                      ],
                    ),
                    ...
                    // Stats
                    Text('Collected Data: ${_accelerometerData.length} accel, ${_gyroscopeData.length} gyro points'),
  ...
  Widget _buildAccelerometerChart(List<SensorDataPoint> data, List<TapEvent> taps) {
    ...
    final seriesList = [
      charts.Series<ChartData, DateTime>(
        ...
        domainFn: (ChartData data, _) => DateTime.fromMillisecondsSinceEpoch(data.timestamp),
